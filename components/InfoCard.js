...
import { BeakerIcon, HeartIcon, StarIcon } from "@heroicons/react/24/solid";

const InfoCard = ({ item }) => {
  const { img, description, location, price, star, title, total } = item;
  ...
  return (
    <div className="flex py-7 px-2 border-b cursor-pointer hover:opacity-80 hover:shadow-lg transition duration-200 ease-out first:border-t">
      <div className="relative h-24 w-40 md:h-52 md:w-80 flex-shrink-0">
        <Image src={img} fill cover className="rounded-2xl" />
      </div>
